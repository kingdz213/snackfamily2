...
export const OrderUI: React.FC<OrderUIProps> = ({
  ...
  const [showTenKmBanner, setShowTenKmBanner] = useState(false);
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    try {
      const ack = window.localStorage.getItem('sf2_10km_ack');
      setShowTenKmBanner(!ack);
    } catch (error) {
      warnDev('[OrderUI] Impossible de lire sf2_10km_ack', error);
    }
  }, []);

  const dismissTenKmBanner = () => {
    setShowTenKmBanner(false);
    if (typeof window === 'undefined') return;
    try {
      window.localStorage.setItem('sf2_10km_ack', '1');
    } catch (error) {
      warnDev('[OrderUI] Impossible de sauvegarder sf2_10km_ack', error);
    }
  };
  ...
  const disableStripeCheckout = !minOk || !deliveryFieldsFilled || !hasGeo || !inRange;
            ...
            <div className="fixed inset-0 flex justify-end pointer-events-none" style={{ zIndex: 9999 }}>
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-gray-50">
                  {showTenKmBanner && (
                    <div className="flex items-start gap-3 bg-black text-white rounded-lg p-3 shadow-md border border-gray-800">
                      <MapPin className="mt-0.5 text-snack-gold" />
                      <div className="flex-1 text-sm space-y-1">
                        <div className="font-bold uppercase tracking-wider text-xs text-gray-200">Zone de livraison</div>
                        <div className="text-gray-100">
                          Livraison possible dans un rayon de <b>{MAX_DELIVERY_KM} km</b> autour du snack.
                        </div>
                        <button
                          type="button"
                          onClick={dismissTenKmBanner}
                          className="text-xs font-bold uppercase tracking-wider text-snack-gold hover:text-white transition-colors"
                        >
                          Compris
                        </button>
                      </div>
                ...
                {cartItems.length > 0 && (
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={isCheckingOut || (paymentMethod === 'stripe' && disableStripeCheckout)}
