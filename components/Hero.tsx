  ...
  return (
    <section id="home" className="relative h-[85vh] w-full overflow-hidden bg-snack-black">
      ...
      <div className="absolute inset-0 z-0">
        ...
        />
        <Embers className="z-10" count={9} maxOpacity={0.16} speed={20} />
        <div className="absolute inset-0 bg-gradient-to-t from-snack-black via-transparent to-snack-black/80 z-20"></div>
      </div>
      ...
      <div className="relative z-20 h-full flex flex-col justify-center items-center text-center px-4 pt-20">
        <motion.div
          ...
          transition={reduceMotion ? { duration: 0 } : { ...motionSafeTransition, duration: 0.8 }}
          className="max-w-4xl"
        >
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-center items-center mt-8">
            <a 
              href="#menu" 
              className={`bg-snack-gold hover:bg-white text-snack-black px-8 py-4 rounded font-display font-bold text-xl uppercase tracking-wider transition-all flex items-center gap-2 shadow-lg hover:shadow-xl transform glow-soft shine-sweep ${reduceMotion ? '' : 'hover:-translate-y-1'}`}
            >
            ...
            <a 
              href="#infos" 
              className={`text-white border-2 border-white/30 hover:border-white px-8 py-3.5 rounded font-display font-bold text-lg uppercase tracking-wider transition-all hover:bg-white/10 ${reduceMotion ? '' : 'hover:-translate-y-0.5'}`}
            >
              Infos & Horaires
            </a>
          </div>
